...
describe('Memory with Processors', () => {
  ...
  it('should apply processors with a real Mastra agent', async () => {
    ...
    console.log(
      'weatherFilteredMessages (MastraDB):',
      JSON.stringify(
        weatherFilteredMessages.map(m => ({
          role: m.role,
    ...
    console.log('weatherFilteredResult:', JSON.stringify(weatherFilteredResult, null, 2));
    console.log('get_weather tool calls:', filterToolCallsByName(weatherFilteredResult, 'get_weather'));
